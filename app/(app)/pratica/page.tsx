...
export default async function PaginaPratica({ searchParams }: PageProps<"/pratica">) {
  ...
  const dems = demandas(foco).map((d) => ({ id: d.id, titulo: d.titulo, de: d.de, canal: d.canal, numero: d.numero, prazo: d.prazo, nivel: d.nivel }));
  const treins = treinos(foco).map((t) => ({ id: t.id, titulo: t.titulo, nivel: t.nivel, tempo: t.tempo }));
  const provs = provas(foco).map((p) => ({
  ...
  const questoes = todasAulas(foco).flatMap((a) => (a.quiz ?? []).map((q) => ({ ...q, aulaId: a.id, aulaTitulo: a.titulo })));
  const titulosAulas = Object.fromEntries(todasAulas(foco).map((a) => [a.id, a.titulo]));
